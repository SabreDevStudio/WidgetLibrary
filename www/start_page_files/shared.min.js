        ...
        debug: function () {
            if (console) {
                ...
                for (var h = 0, k = Object.keys(c).sort(), i = null;
                     ...
                     h++) {
                    var j = k[h];
                    if (this.config.container && (typeof(j) == "string") && !j.match(f)) {
                        continue
                    }
                    var g = c[j];
...
CQ.shared.Form = function () {
    ...
    var f = function (h) {
        ...
        var g = function (k) {
            ...
            if (k.nodeName.toLowerCase() == "select" || k.nodeName.toLowerCase() == "input" || k.nodeName.toLowerCase() == "textarea" || k.nodeName.toLowerCase() == "button") {
                return
            }
            for (var j = 0;
    ...
    return {
        searchArray: function (h, g, k) {
            ...
            var g = [];
            var h = function (n, o) {
                var m = n.getAttribute("name");
                var p = n.nodeName.toLowerCase();
                var r;
                if (p == "input" || p == "textarea") {
                    var l = d(n, "type") ? n.getAttribute("type").toLowerCase() : "text";
                    if (l == "button" || l == "submit" || l == "reset") {
                        return
                    }
                    r = CQ.shared.Form.searchArray(g, "value", m);
                    if (!r) {
                        g.push({
                            text: CQ.shared.Form.getLabelForField(n, j),
                            value: m,
                            name: m,
            ...
            var i = function (m, l) {
                if (m.nodeName.toLowerCase() == "div" && $CQ(m).hasClass("section")) {
                    l = true
                }
                if (m.getAttribute && m.getAttribute("name")) {
                    h(m, l)
                }
                for (var k = 0;
                     m.childNodes && k < m.childNodes.length;
                     k++) {
                    var n = m.childNodes[k];
                    if (n.nodeType == 1) {
                        i(n, l)
                    }
                }
