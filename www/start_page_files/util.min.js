...
function record(g, a, j, d, e) {
    ...
    for (var c = record.callbacks.length - 1;
         c >= 0;
         c--) {
        CQ_Analytics.registerAfterCallback(h(record.callbacks[c]), 150 - c);
...
CQ_Analytics.Sitecatalyst = {
    events: [], trackVars: [], settings: [], stores: CQ_Analytics.StoreRegistry.getStores(), hasEvent: function (e) {
        var c = CQ_Analytics.StoreRegistry.getStore("eventdata");
        if (c) {
    ...
    }, eraseTrackVars: function (b) {
        for (var a = 0;
             a < this.trackVars.length;
             a++) {
            var c = this.trackVars[a];
            if (c !== "events") {
                if (b) {
                    b[c] = s[c]
                }
        ...
        var a = {trackDownloadLinks: s.trackDownloadLinks, trackExternalLinks: s.trackExternalLinks};
    ...
    }, trackLink: function (d) {
        var e = d.options.obj || "", b;
        ...
        for (var b = 0;
             b < c.length;
             b++) {
            if (c[b].getAttribute("data-tracking")) {
                this.processDataAttribute(c[b].getAttribute("data-tracking"), a)
            } else {
                if (c[b].getAttribute("record")) {
                    this.processRecordAttribute(c[b].getAttribute("record"), a)
                }
            }
        }
    ...
    }, storeTrackingData: function (b) {
        var f = CQ_Analytics.StoreRegistry.getStore("eventdata");
        if (f) {
            var c = this.toObj(f.getProperty("events").split("\u2026"));
            var d = b[0];
            c[d] = d;
            var e = b[1];
            var a = f;
            ...
            if (window.console) {
                window.console.warn("EventData Store is not defined")
            }
        }
    }, toArr: function (b) {
        var a = [];
