/*1425941480,,JIT Construction: v1633075,en_US*/
...
try {
    (function (a, b, c, d) {
        ...
        function o(v, w) {
            var x = 'fb' + Math.random().toString().replace('.', ''), y = b.createElement('form');
            y.method = 'post';
            y.action = v;
            y.target = x;
            y.acceptCharset = 'utf-8';
            y.style.display = 'none';
            var z = !!(a.attachEvent && !a.addEventListener), aa = z ? '<iframe name="' + x + '">' : 'iframe', ba = b.createElement(aa);
        ...
        function p(v, w, x) {
            ...
            if (x && typeof x === 'object')for (var z in x)if (x.hasOwnProperty(z)) {
                var aa = x[z], ba = (aa === null) ? 'null' : typeof aa;
                if (ba in {number: 1, string: 1, boolean: 1}) {
    ...
    new Image().src = "http:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m=' + encodeURIComponent('{"error":"LOAD", "extra": {"name":"' + e.name + '","line":"' + (e.lineNumber || e.line) + '","script":"' + (e.fileName || e.sourceURL || e.script) + '","stack":"' + (e.stackTrace || e.stack) + '","revision":"1633075","message":"' + e.message + '"}}');
}